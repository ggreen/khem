    ...
    implements IdentifierHardDependencies, AromaticityTyper
{
        ...
        BondDynamicResult bonds)
    {
        ...
        if (useAromaticityModel)
        {
            ...
            {
            		if(atomIdx >= numberOfElectrons.length)
            			continue;
    ...
    */
        ...
        boolean[] aromBonds)
    {
        ...
        while (ait.hasNext())
        {
            ...
            if (aIndex < isRoot.length && isRoot[aIndex])
            {
                if (logger.isDebugEnabled())
                {
        ...
        while (ait.hasNext())
        {
            ...
            // check only for root atoms
            if (atom.getIndex() < isRoot.length &&
            	   isRoot[atom.getIndex()])
            {
        ...
        boolean[] aromBonds)
    {
        ...
        while (nait.hasNext())
        {
            ...
            {
            		if(atom.getIndex() >= numberOfElectrons.length)
            			continue;
    ...
    private void excludeSmallRing(Molecule mol)
    {
        ...
        while (ait.hasNext())
        {
            ...
            if (atom.getIndex() < isRoot.length && isRoot[atom.getIndex()])
            {
                ...
                while (nait1.hasNext())
                {
                    ...
                    nbr1.getIndex() < potentiallyAromatic.length &&
                    potentiallyAromatic[nbr1.getIndex()])
                    {
                        ...
                        while (nait2.hasNext())
                        {
                            ...
                            if(nbr2.getIndex() >= potentiallyAromatic.length )
                            	continue;
    ...
    private void markPotentiallyAromatic(Molecule mol)
    {
        ...
        for (idx = 0, k = 0; k < smarts.size(); k++, idx++)
        {
            ...
            if (tmpSP.match(mol))
            {
            	...
            	List<int[]> matchList = new ArrayList<int[]>(tmpSP.getMatches());

                //        System.out.print("mlist:");
                for (int[] itmp : matchList)
				{
                   ...
                   // itmp = matchList.get(m);
                    ...
                    if(itmp[0] < potentiallyAromatic.length)
                    {
                    		potentiallyAromatic[itmp[0]] = true;
                    }

                    BasicIntInt iiMinMaxE = minMaxElectrons.get(idx);
                    if(iiMinMaxE != null && itmp[0] < numberOfElectrons.length)
                    {
    ...
    private void propagatePotArom(Molecule mol)
    {
        ...
        while (ait.hasNext())
        {
            ...
            if (atom.getIndex() < potentiallyAromatic.length && potentiallyAromatic[atom.getIndex()])
            {
    ...
    private void propagatePotArom(Atom atom)
    {
        ...
        while (nait.hasNext())
        {
            ...
            nbr.getIndex() < potentiallyAromatic.length &&
        ...
        if (count < 2)
        {
            if(atom.getIndex() < potentiallyAromatic.length)
            	{
            ...
            if (count == 1)
            {
                ...
                while (nait.hasNext())
                {
                    ...
                    if (nbr.getIndex() < potentiallyAromatic.length &&
                    		BondInRing.isInRing(nait.actualBond()) &&
                            potentiallyAromatic[nbr.getIndex()])
                    {
    ...
    private void selectRootAtoms(Molecule mol, boolean avoidInnerRingAtoms)
    {
        ...
        while (bit.hasNext())
        {
            ...
            if (BondIsClosure.isClosure(bond))
            {
                            ...
                            if (rootAtomNumber < 2)
                            {
                                        ...
                                        ringAtomIdx++)
                                {
                                    ...
                                    else
                                    {
                                    	...
                                    	if(ringAtomIdx < ringIndices.length)
                                    	{
                                    		 if (ringIndices[ringAtomIdx] < isRoot.length &&
                                             isRoot[ringIndices[ringAtomIdx]] ==
                                                     true)
                                             {
                ...
                if (rootAtom < isRoot.length && isRoot[rootAtom] == true)
                {
                    if (logger.isDebugEnabled())
                    {
        ...
        boolean[] aromBonds)
    {
        ...
        if(atom.getIndex() < potentiallyAromatic.length &&
        atom.getIndex() < isVisited.length)
        {
        ...
        int aIndex = atom.getIndex();
        if( aIndex >= isVisited.length)
        		return false;
        	
        	isVisited[aIndex] = true;
        ...
        isVisited[aIndex] = false;

        if (result)
        {
            aromAtoms[aIndex - 1] = true;
        }

        if(numberOfElectrons[aIndex] == null)
        		return false;
        
        electrons.intValue1 -= (numberOfElectrons[aIndex]).intValue1;
        electrons.intValue2 -= (numberOfElectrons[aIndex]).intValue2;

        if (logger.isDebugEnabled())
        {
            System.out.println("electron contribution: " + aIndex +
                " electrons " + electrons + " elAtom:" +
                numberOfElectrons[aIndex] + " aromatic:" +
                aromAtoms[aIndex - 1]);
        }
